...
* **EYEBROW RESTRAINT (mandatory, the #1 violated rule in production tests).** An "eyebrow" is the small uppercase wide-tracking label sitting above a section headline. Every AI-built site puts an eyebrow above EVERY section header, producing the same templated rhythm. Hard rule:
...
1. **Image-generation tool first.** If ANY image-gen tool is available, you MUST use it to create section-specific assets: hero photography, product shots, texture backgrounds, mood images. Generate at the right aspect ratio for the section. Do not skip this step because hand-rolled CSS feels faster.
...
3. **Last resort: tell the user.** If neither is possible, do NOT fill the page with hand-rolled SVG illustrations or div-based "fake screenshots." Instead, leave clearly-labeled placeholder slots and say: *"This page needs real images at: [list of placements]. Please generate or provide them."*
...
* **COPY SELF-AUDIT (mandatory before ship):** Before declaring any task done, re-read every visible string on the page. Flag any string that is:
  ...
  Rewrite every flagged string. If unsure, replace it with a plain functional sentence. AI-generated cute copy is worse than boring copy.
...
* **Liquid Glass / Glassmorphism:** Appropriate for premium consumer, Apple-adjacent, luxury brand, or media-overlay vibes. When used, go beyond `backdrop-blur`: add a 1px inner border and subtle inner shadow for physical edge refraction. Provide a solid-fill fallback under `prefers-reduced-transparency`.
...
* **Perpetual Micro-Interactions** (Pulse, Typewriter, Float, Shimmer, Carousel): Use when `MOTION_INTENSITY > 5` AND the section actively benefits from motion. Apply Spring Physics (`type: "spring"`) - no linear easing.
* **"Motion claimed, motion shown."** If `MOTION_INTENSITY > 4`, the page must actually move.
* **MOTION MUST BE MOTIVATED (mandatory).** Before adding any animation, ask: "what does this animation communicate?" Valid answers: hierarchy, storytelling, feedback, state transition. Invalid: "it looked cool".
...
* **Tailwind `dark:` variant** (default for utility-first projects): every color utility paired with its dark variant (`bg-white dark:bg-zinc-950`).
* **CSS variables** (for shadcn/ui, Radix Themes, or component libraries with theming): define semantic tokens and swap values under `[data-theme="dark"]` or `@media (prefers-color-scheme: dark)`.
...
* **NO 3-column equal feature cards.** Use 2-column zig-zag, asymmetric grid, or scroll-pinned alternative.

### 9.D Content & Data ("Jane Doe" Effect)

* **NO generic names.** "John Doe", "Sarah Chan" → use creative, realistic, locale-appropriate names.
* **NO generic avatars.** No SVG "egg" or Lucide user icons → use believable photo placeholders.
...
* **NO filler verbs.** "Elevate", "Seamless", "Unleash" → concrete verbs only.

### 9.E External Resources & Components

* **NO hand-rolled SVG icons.** Use Phosphor / HugeIcons / Radix / Tabler.
* **NO div-based fake screenshots.** Use real images, generated images, or skip the preview.
* **shadcn/ui customization:** Allowed, but NEVER in default state.

### 9.F EM-DASH BAN (the single most-violated Tell)

**Em-dash (`—`) is COMPLETELY banned.** It is the LLM's signature stylistic crutch and the #1 visual Tell in production tests.

* **Banned in headlines, eyebrows, labels, pills, button text, image captions, nav items, body copy, quote attribution.** Replace with line breaks, columns, hairlines, or restructured sentences.
* The ONLY permitted dash characters on the page are: regular hyphen `-` and minus sign in math.
...
Run this matrix before outputting code. If any box fails, the page is not done.
...
- [ ] **ZERO em-dashes (`—`) anywhere on the page** (Section 9.G)?
...
- [ ] **Button Contrast Check**: every CTA text is readable against its background (WCAG AA)?
- [ ] **CTA Button Wrap**: no CTA label wraps to 2+ lines at desktop?
- [ ] **Form Contrast Check**: form inputs, placeholders, labels all pass WCAG AA?
- [ ] **Serif discipline**: if a serif is used, NOT Fraunces or Instrument_Serif, OR explicitly justified?
- [ ] **Premium-consumer palette check**: if premium-consumer brief, palette NOT the beige+brass default?
...
- [ ] **EYEBROW COUNT**: count eyebrows across all sections. Count ≤ ceil(sectionCount / 3)?
- [ ] **Split-Header Ban**: no "left big headline + right small explainer" pattern as section header?
- [ ] **Zigzag Alternation Cap**: no 3+ consecutive sections with same image+text-split layout?
- [ ] **No Duplicate CTA Intent**: no two CTAs with same intent on page?
- [ ] **Logo wall = logo only**: no industry / category labels below logos?
- [ ] **Copy Self-Audit**: every visible string re-read, no AI hallucinations shipped?
- [ ] **Motion motivated**: every animation justified (hierarchy / storytelling / feedback / state transition)?
- [ ] **Marquee max-one-per-page**: no two horizontal marquees?
...
- [ ] **Real images used** (no div-based fake screenshots, no hand-rolled SVGs)?
- [ ] **No em-dashes anywhere** (headlines, buttons, captions, alt text)?
...
- [ ] **No AI Tells** from Section 9 (Inter default, AI-purple, three-equal cards, generic names)?
...
**When building:**
1. Read the brief → output a one-line design read (Section 0.B)
2. Set the three dials based on design read (Section 1)
3. Pick a design system or aesthetic (Section 2)
4. Use default stack (Section 3) unless brief says otherwise
5. Apply bias-correction directives (Section 4)
6. Audit against pre-flight check (Section 10)

**When redesigning:**
1. Detect the mode (preserve vs overhaul) (Section 11)
2. Audit before touching (brand tokens, IA, patterns)
3. Apply modernisation levers in order
4. Preserve IA, content, and SEO
5. Audit against pre-flight check

**Golden rule:** Do not default. Read the room, then pull what fits.
